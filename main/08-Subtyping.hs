...
-- | The type language. Replaces the Peano naturals from module 05
-- with a numeric tower: @Nat <: Int <: Real@.
data Type
  = FuncTy Type Type
  | PairTy Type Type
  | UnitTy
...
-- Environment
--
-- The typechecker's context tracks names (for resolving named
-- variables), types (for typechecking), and values (for quoting).
...
-- Typechecker
--
-- Bidirectional typechecking with elaboration. The key change in this module:
-- the 'subTactic' uses 'isSubtypeOf' instead of equality, so a term that
-- synthesizes @Nat@ can be used where @Int@ is expected.
...
-- against the type's fields via a 'Map'. Field order is irrelevant.
--
--         for each i  Γ ⊢ tᵢ ⇐ Tᵢ
-- ─────────────────────────────────────── Record⇐
-- Γ ⊢ { lᵢ = tᵢ} ⇐ { lᵢ : Tᵢ (i ∈ I..n) }
...
--
-- Synthesize the record's type, then look up the projected field by name.
--
-- Γ ⊢ t₁ ⇒ { lᵢ : Tᵢ (i ∈ I..n) }
-- ─────────────────────────────── Get⇒
--       Γ ⊢ Get lⱼ t₁ ⇒ Tⱼ
...
-- Evaluator
--
-- Evaluation maps 'Syntax' to 'Value' under an environment. Subtyping is a
-- typechecking concern and does not affect evaluation.
